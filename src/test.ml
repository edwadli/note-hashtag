...
let rec string_of_fdef f = match f with
	|[] -> ""
	|FunDef(x, y, z) :: rest -> String.concat " " [Bytes.to_string x; string_of_var_list y; string_of_expr z; string_of_fdef rest]

and string_of_var_list v = match v with
	[] -> ""
	|s::rest -> String.concat " " [Bytes.to_string s; string_of_var_list rest]

let rec string_of_incl_list p = match p with
	[] -> "[]"
	|s::rest -> String.concat " " [Bytes.to_string s; string_of_incl_list rest] 

let string_of_prog_struc p = match p with
	|(a, b, c)-> String.concat " " ["INCLUDES:"; string_of_incl_list a; "\nFDEF:"; string_of_fdef b; "\nEXPR: "; string_of_exp_list c] 
	

let print_ast_dump =
  let lexbuf = Lexing.from_channel stdin in
  let prog = Parser.program Scanner.token lexbuf
	in let p = string_of_prog_struc prog
	in print_endline p
(* string_of_expr expr "" *)
